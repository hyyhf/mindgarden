...
test('the standalone release exposes an explicit Harness source-authority gate', () => {
  ...
  assert.match(syncScript, /\['UI build artifacts', join\(UI_SOURCE, 'lib'\), join\(UI_TARGET, 'lib'\)\]/)
  assert.match(syncScript, /\['src', 'tests', 'skills', 'lib'\]/)
  assert.match(syncScript, /entry\.name !== 'tsconfig\.tsbuildinfo'/)
  assert.match(syncScript, /!entry\.name\.endsWith\('\.map'\)/)
  assert.match(syncScript, /replace\(\/\[ \\t\]\+\$\/gmu, ''\)/)
})
