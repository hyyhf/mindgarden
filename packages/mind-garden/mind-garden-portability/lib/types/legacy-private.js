...
import { decryptLegacyField, deterministicLegacyUuid, legacyBlob, legacyFiniteNumber, legacyInteger, legacyMilliseconds, legacyOptionalText, legacyText, readLegacyRows, } from "./legacy-values.js";
...
function invalid(label, cause) {
    return new MindGardenPortabilityError('invalid-backup', `Original Mind Garden ${label} is unsupported`, cause === undefined ? undefined : { cause });
}
function toBackupRecord(id, value) {
    return { id, value: JSON.parse(JSON.stringify(value)) };
}
...
function privateText(value, key, aad, label, allowEmpty = false) {
    ...
    try {
        if (plaintext.length > MAX_LEGACY_FIELD_BYTES)
            throw invalid(label);
...
function convertConcerns(context) {
  ...
  `, MAX_LEGACY_RECORDS).map((row) => {
        ...
        if (status === 'converted' && convertedJournal === null)
            throw invalid('converted concern journal');
...
function convertContemplations(context) {
  ...
  `, MAX_LEGACY_RECORDS).map((row) => {
        ...
        const confirmedAt = status === 'confirmed'
            ? Math.max(createdAt, row.confirmed_at === null
                ? originalUpdatedAt
    ...
    for (const name of ['philosophy-notebook.md', 'personal-philosophy.md']) {
        ...
        if (bytes === null)
            continue;
        try {
            if (bytes.length > MAX_LEGACY_FIELD_BYTES)
                throw invalid(`${name} file`);
...
function convertPrinciples(context) {
  ...
  `, MAX_LEGACY_RECORDS);
    const versionsByPrinciple = new Map();
  ...
  `, MAX_LEGACY_RECORDS).map((row) => {
        ...
        if (currentVersion < 1 || sourceRows.length !== currentVersion)
            throw invalid('principle version history');
        const versions = sourceRows.map((versionRow, index) => {
            const number = legacyInteger(versionRow.version, 'principle version number');
            if (number !== index + 1)
                throw invalid('principle version sequence');
        ...
        if (status !== latest.content.status)
            throw invalid('principle current status');
        return reflectionRecord({
...
function currentMemoryKind(value) {
    if (value === 'fact')
        return 'episode';
    if (value === 'preference' || value === 'support_preference')
        return 'support-preference';
    if (value === 'identity' || value === 'value' || value === 'decision'
        || value === 'emotion' || value === 'episode')
        return value;
  ...
  `, MAX_LEGACY_RECORDS).map((row) => {
        ...
        const convertedSources = (sources.get(legacyId) ?? []).map((source) => {
            ...
            const quote = messageId === null
                ? ''
                : optionalPrivateText(source.quote_enc, context.key, `memory_source:${legacyId}:${messageId}`, 'memory source quote');
            return quote.length > 0 && messageId !== null
                ? { sessionId, messageId, evidenceQuote: quote }
                : { sessionId };
        });
        if (convertedSources.length === 0)
            convertedSources.push({ sessionId: context.sourceSessionId });
 ...
 */
export function convertLegacyPrivateCollections(database, key, workspaceId, createdAt, files) {
    const context = {
        database,
        key,
        workspaceId,
...
//# sourceMappingURL=legacy-private.js.map