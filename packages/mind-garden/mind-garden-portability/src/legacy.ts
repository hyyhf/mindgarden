...
import {
  ...
  legacyInteger,
  legacyMilliseconds,
  legacyPositiveInteger,
  legacyText,
} from './legacy-values.ts'
import { convertLegacyPrivateCollections } from './legacy-private.ts'
...
interface LegacyPhotoRow {
  ...
  readonly media_type: SQLOutputValue
  readonly width: SQLOutputValue
  readonly height: SQLOutputValue
  readonly image_enc: SQLOutputValue
  readonly particle_config_enc: SQLOutputValue
...
function assertLegacyPassphrase(passphrase: string): void {
  ...
  if (codePoints < LEGACY_MIN_PASSPHRASE_CODE_POINTS
    ...
    throw new MindGardenPortabilityError(
      'invalid-passphrase',
      `Original Mind Garden passphrase must contain ${LEGACY_MIN_PASSPHRASE_CODE_POINTS}-${LEGACY_MAX_PASSPHRASE_CODE_POINTS} characters`,
    )
  }
}

function decryptLegacyPackage(data: Uint8Array, passphrase: string, maxPlaintextBytes: number) {
  ...
  try {
    ...
    throw new MindGardenPortabilityError(
      'invalid-backup',
      'Original Mind Garden package could not be authenticated',
      { cause: error },
    )
  } finally {
    bytes.fill(0)
...
function mediaType(value: SQLOutputValue): ImageMediaType {
...
function readLegacyPhotoRows(database: DatabaseSync): readonly LegacyPhotoRow[] {
  ...
  return rows as LegacyPhotoRow[]
}

async function convertLegacyProfile(
  ...
  try {
    ...
    quickCheck(database)
    const privateCollections = convertLegacyPrivateCollections(
      database,
      dataKey,
      workspaceId,
      createdAt,
    ...
    for (const row of readLegacyPhotoRows(database)) {
      ...
      const image = decryptLegacyField(
        legacyBlob(row.image_enc, 'photo image'),
        dataKey,
        `photo_story:${legacyId}:image`,
      )
      const configBytes = decryptLegacyField(
        legacyBlob(row.particle_config_enc, 'photo particle configuration'),
        dataKey,
      ...
      try {
        ...
        const record = decodeStoredMediaRecord({
          ...
          particleConfig: JSON.parse(configBytes.toString('utf8')) as unknown,
          observation: null,
          turns: [],
          quickReplies: [],
          modelRuns: [],
        ...
        if (existing !== undefined
          ...
          throw new MindGardenPortabilityError(
            'invalid-backup',
            'Original Mind Garden repeats photo bytes with conflicting metadata',
          )
        }
        ...
        media.push({ id, value: JSON.parse(JSON.stringify(record)) as JsonValue })
      } catch (error) {
        if (error instanceof MindGardenPortabilityError) throw error
        throw new MindGardenPortabilityError(
          'invalid-backup',
          'Original Mind Garden photo record is unsupported',
          { cause: error },
        )
      } finally {
        image.fill(0)
 ...
 */
export function isLegacyMindGardenPackage(data: Uint8Array): boolean {
 ...
 */
export async function loadLegacyMindGardenBackup(
  data: Uint8Array,
  passphrase: string,
  maxPlaintextBytes: number,
): Promise<MindGardenLegacyBackup> {
