...
function policyVisible(agent, text) {
	return agent.session.deriveMessages().some((message) => message.source.kind === "plugin" && message.source.plugin === "mind-garden-dialogue" && message.source.form === "snapshot" && message.content.some((block) => block.type === "text" && block.text === text));
}
function signalIsCurrentlyAborted(signal) {
...
function apply(ctx) {
	ctx.on("agent/pre-step", async ({ agent, step, signal }, next) => {
		...
		let authorizedJournals = [];
		if (observed.privacy === "durable") {
			const query = decision.messages.filter((message) => message.source.kind === "user").flatMap((message) => message.content.flatMap((block) => block.type === "text" ? [block.text] : [])).join("\n").trim();
			const reflection = ctx.get("mindGardenReflection");
			if (query !== "" && reflection !== void 0) try {
				const authorized = await reflection.authorizedContext(agent, { query });
				if (authorized.ok && authorized.value.retrievableJournals.length > 0) authorizedJournals = authorized.value.retrievableJournals;
				else if (!authorized.ok) ctx.logger.warn(`mind-garden-dialogue: authorized journal context unavailable (${authorized.error.code})`);
			} catch {
		...
		if (!policyVisible(agent, text)) additions.push(createUserMessage({
			content: [{
				type: "text",
				text
			}],
...
//#endregion
export { apply, inject, name, renderAuthorizedJournalContext, renderMindGardenDialoguePolicy };
