...
function policyVisible(agent, text) {
    return agent.session.deriveMessages().some(message => message.source.kind === 'plugin'
 ...
 */
export function renderAuthorizedJournalContext(journals) {
    return [
...
export function apply(ctx) {
    ctx.on('agent/pre-step', async ({ agent, step, signal }, next) => {
        ...
        if (observed === null)
            return decision;
        if (!observed.modelDisclosureAccepted)
            return { kind: 'reject' };
        ...
        if (signalIsCurrentlyAborted(signal))
            return decision;
