...
import { z } from 'zod'
import type {
  MindGardenStarCardAnalysis,
  MindGardenStarCard,
  MindGardenStarDeck,
  MindGardenStarEvidenceType,
  MindGardenStarObserverTone,
  MindGardenStarProfile,
  MindGardenStarTrait,
  MindGardenStarTraitKind,
} from './types.ts'
...
function firstPersonQuestion(value: string): boolean {
  return /我/u.test(value) || /\b(?:I|me|my|mine)\b/iu.test(value)
}

function containsInternalReference(value: string, sources: readonly StarObserverSource[]): boolean {
 ...
 */
export function buildStarObserverDialogueEnvelope(
  card: MindGardenStarCard,
  content: string,
  quickReplyKind: '' | 'deepen' | 'shift' | 'correct',
  maxBytes: number,
): StarObserverEnvelope | null {
  const evidence: StarObserverSource[] = card.evidence.map((item, index) => ({
    key: `e${index + 1}`,
 ...
 */
export function decodeStarObserverDialogueOutput(
  raw: string,
  evidence: readonly StarObserverSource[],
  cardKind: MindGardenStarCard['cardKind'],
): StarObserverDialogueProposal | null {
  ...
  if (containsInternalReference(visible, evidence)) return null
 ...
 */
export function buildStarObserverEnvelope(
  profile: MindGardenStarProfile,
  traits: readonly MindGardenStarTrait[],
  deck: MindGardenStarDeck,
  question: string,
  tone: MindGardenStarObserverTone,
  evidence: readonly StarObserverSource[],
  maxBytes: number,
): StarObserverEnvelope | null {
  const payload = {
 ...
 */
export function decodeStarObserverOutput(
  raw: string,
  evidence: readonly StarObserverSource[],
): StarObserverProposal | null {
  const trimmed = raw.trim()
  const fenced = /^```(?:json)?\s*\r?\n([\s\S]*?)\r?\n```$/iu.exec(trimmed)
  let value: unknown
  try {
