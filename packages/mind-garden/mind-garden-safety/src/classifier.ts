...
export function assessMindGardenInput(
  text: string,
  locale: MindGardenSafetyLocale = detectMindGardenSafetyLocale(text),
): MindGardenSafetyAssessment {
  ...
  return clauses
    ...
    .reduce((highest, assessment) => assessment.level > highest.level ? assessment : highest,
      result(0, 'ordinary', [], 0, locale))
}
