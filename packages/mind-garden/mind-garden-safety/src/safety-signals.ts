...
const SELF_TERMS = ['我', '自己', '本人', 'i', 'me', 'myself', 'my life'] as const
...
const ACTION_TAKEN_TERMS = ['已经', '刚刚', '刚才', '正', 'just', 'already'] as const
...
function quotedPolicyText(value: string): string {
  return [...value.matchAll(/[“"「『]([^”"」』]+)[”"」』]/gu)]
    .map(match => match[1] ?? '')
    .join(' ')
}

function unquotedPolicyText(value: string): string {
  return value.replace(/[“"「『][^”"」』]+[”"」』]/gu, ' ')
}

function hasDirectRiskEvidence(text: string): boolean {
 ...
 */
export function hasPolicyPhrase(value: string, phrase: string): boolean {
 ...
 */
export function hasAnyPolicyPhrase(value: string, phrases: readonly string[]): boolean {
 ...
 */
export function extractMindGardenSafetySignals(clause: string): MindGardenSafetySignals {
