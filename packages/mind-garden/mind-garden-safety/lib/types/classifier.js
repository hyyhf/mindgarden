...
function splitSafetyClauses(text) {
    return text
        .split(/(?:[。！？!?；;\n]+|(?<!不)(?:但是|不过|然而|可是|但|却)|\b(?:but|however|although|though|yet)\b)/iu)
 ...
 */
export function normalizeMindGardenSafetyText(text) {
...
export function assessMindGardenInput(text, locale = detectMindGardenSafetyLocale(text)) {
    ...
    const clauses = splitSafetyClauses(normalized).flatMap((clause) => {
    ...
    return clauses
        .map(clause => assessClause(clause, locale))
        .reduce((highest, assessment) => assessment.level > highest.level ? assessment : highest, result(0, 'ordinary', [], 0, locale));
}
/**
 ...
 */
export function recoverMindGardenSafetyState(current, previous, text) {
    if (current.level > 0 || previous === undefined || previous.level === 0)
        return current;
    const normalized = normalizeMindGardenSafetyText(text);
    const safetyConfirmed = SAFETY_CONFIRMED_TERMS.some(term => normalized.includes(term));
    if (previous.level === 3) {
        return safetyConfirmed
            ? result(2, 'support-follow-up', ['immediate-danger-reduced'], 0, current.locale)
            : result(3, 'support-follow-up', ['urgent-follow-up'], previous.normalTurns, current.locale);
    }
    if (previous.level === 2) {
        return safetyConfirmed
            ? result(1, 'support-follow-up', ['safety-confirmed'], 0, current.locale)
            : result(2, 'support-follow-up', ['safety-follow-up'], previous.normalTurns, current.locale);
    }
    const normalTurns = previous.normalTurns + 1;
    return normalTurns >= 2
        ? result(0, 'ordinary', [], 0, current.locale)
        : result(1, 'support-follow-up', ['safety-follow-up'], normalTurns, current.locale);
}
