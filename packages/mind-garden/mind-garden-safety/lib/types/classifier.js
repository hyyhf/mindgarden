 ...
 */
export function assessMindGardenInput(text) {
    const normalized = normalizeMindGardenSafetyText(text);
    if (normalized.includes('不想活在')
        || matches(NEGATED_RISK_PATTERNS, normalized)
        || matches(BENIGN_CONTEXT_PATTERNS, normalized))
        return result(0, 'ordinary', []);
    if (matches(URGENT_PATTERNS, normalized))
        return result(3, 'urgent', ['immediate-danger']);
    if (matches(ABUSE_DANGER_PATTERNS, normalized))
        return result(2, 'abuse-danger', ['abuse-or-child-safety']);
    ...
    if (matches(HIGH_RISK_PATTERNS, normalized))
        return result(2, 'high-risk', ['self-or-other-harm']);
    if (matches(VULNERABLE_PATTERNS, normalized))
        return result(1, 'vulnerable', ['severe-distress']);
    return result(0, 'ordinary', []);
}
/**
