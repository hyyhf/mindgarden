...
export const PHOTO_OBSERVATION_PROMPT_VERSION = 'mind-garden-photo-observe-v1' as const
...
function firstPerson(value: string): boolean {
...
function safeVisibleCopy(values: readonly string[]): boolean {
 ...
 */
export function decodePhotoObservationOutput(raw: string): PhotoObservationProposal | null {
  ...
  if (questionMarks !== 1 || !/[?？]$/u.test(parsed.data.opening)) return null
  ...
  if (!safeVisibleCopy(visible)) return null
 ...
 */
export function decodePhotoDialogueOutput(raw: string): PhotoDialogueProposal | null {
