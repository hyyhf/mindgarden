...
import type {
  ...
  MindGardenMemoryRelationshipType,
...
export function buildExtractionEnvelope(
  ...
  const eligible = messages.flatMap((message): ExtractionTranscriptRow[] => {
    ...
    return [{ id: message.id, role: message.role as 'user' | 'assistant', text }]
  })
  const transcript: ExtractionTranscriptRow[] = []
  for (const row of [...eligible].reverse()) {
    const next = [row, ...transcript]
    if (jsonBytes(next) <= maxTranscriptBytes) transcript.unshift(row)
  }
  const selectedMemories: ExtractionComparableMemory[] = []
  for (const memory of [...memories].reverse()) {
    const next = [memory, ...selectedMemories]
    if (jsonBytes(next) <= maxMemoryBytes) selectedMemories.unshift(memory)
  }
 ...
 */
export function decodeExtractionOutput(raw: string): ExtractionProposal[] | null {
  ...
  const proposals = output.data.memories.flatMap((candidate): ExtractionProposal[] => {
    const parsed = proposalSchema.safeParse(candidate)
    if (!parsed.success) return []
    return [parsed.data as ExtractionProposal]
  })
