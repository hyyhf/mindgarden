 ...
 */
export function decodeStoredRecord(value) {
    const discriminator = z.looseObject({ recordType: z.string() }).parse(value).recordType;
    if (discriminator === 'memory')
        return storedMemorySchema.parse(value);
    if (discriminator === 'retrieval-audit')
        return storedAuditSchema.parse(value);
    if (discriminator === 'extraction-run')
        return storedExtractionRunSchema.parse(value);
    if (discriminator === 'automation-policy')
