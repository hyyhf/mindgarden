...
const FORBIDDEN_INFERENCE_PATTERN = new RegExp('(?:diagnos(?:is|ed)|personality disorder|attachment style|trauma type|subconscious|risk score|'
    + '诊断|患有|人格障碍|依恋类型|创伤类型|潜意识|危险等级|风险评分)', 'iu');
...
function resolveConfig(config) {
    ...
    return Object.freeze({
        ...
        maxExtractionOutputTokens: positiveSafeInteger(config.maxExtractionOutputTokens ?? DEFAULT_MAX_EXTRACTION_OUTPUT_TOKENS, 'maxExtractionOutputTokens'),
        extractionProvider,
        extractionModel,
    });
}
    ...
    return class MindGardenMemoryService extends _classSuper {
        ...
        constructor(ctx, config) {
            ...
            ctx.on('agent/pre-step', async ({ agent, step, signal }, next) => {
                ...
                if (state === null || state.privacy !== 'durable')
                    return decision;
                try {
                    const recall = await this.enqueue(async () => await this.prepareRecall(agent, userQuery(decision.messages)));
                    if (recall === null)
                        return decision;
                    return {
                        kind: 'enter',
                ...
                try {
                    ...
                    if (!alreadyApplied) {
                        ...
                        if (request.temporaryDays === undefined)
                            ...
                            replacement.expiresAt = now + request.temporaryDays * DAY_MS;
        ...
        async automaticExtractionDue(agent) {
            if (this.accessFailure(agent) !== null)
                return false;
            const records = await this.readRecords();
        ...
        async prepareExtraction(agent, request, trigger, automaticSourceMessageIds) {
            ...
            const envelope = buildExtractionEnvelope(messages, memories, this.options.maxExtractionInputBytes, this.options.maxExtractionMemoryBytes);
