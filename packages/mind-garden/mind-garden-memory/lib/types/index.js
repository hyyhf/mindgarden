...
import { BlockAssembler, createUserMessage, HarnessError, isAgentLoopRequest, MessageId, ReasoningEffortId, } from '@deepseek-ai/dsh-llm';
...
import { forbiddenInferenceKind, interpretCorrectionDecision } from "./text-policy.js";
...
function resolveConfig(config) {
    ...
    return Object.freeze({
        ...
        ...(config.maxExtractionOutputTokens === undefined
            ? {}
            : {
                maxExtractionOutputTokens: positiveSafeInteger(config.maxExtractionOutputTokens, 'maxExtractionOutputTokens'),
            }),
        extractionProvider,
        extractionModel,
    });
...
let MindGardenMemoryService = (() => {
    ...
    return class MindGardenMemoryService extends _classSuper {
        static {
            ...
            __esDecorate(this, null, _propose_decorators, { kind: "method", name: "propose", static: false, private: false, access: { has: obj => "propose" in obj, get: obj => obj.propose }, metadata: _metadata }, null, _instanceExtraInitializers);
            ...
            __esDecorate(this, null, _resolveRelationship_decorators, { kind: "method", name: "resolveRelationship", static: false, private: false, access: { has: obj => "resolveRelationship" in obj, get: obj => obj.resolveRelationship }, metadata: _metadata }, null, _instanceExtraInitializers);
            __esDecorate(this, null, _listRevisions_decorators, { kind: "method", name: "listRevisions", static: false, private: false, access: { has: obj => "listRevisions" in obj, get: obj => obj.listRevisions }, metadata: _metadata }, null, _instanceExtraInitializers);
            __esDecorate(this, null, _automationPolicy_decorators, { kind: "method", name: "automationPolicy", static: false, private: false, access: { has: obj => "automationPolicy" in obj, get: obj => obj.automationPolicy }, metadata: _metadata }, null, _instanceExtraInitializers);
            __esDecorate(this, null, _setAutomationPolicy_decorators, { kind: "method", name: "setAutomationPolicy", static: false, private: false, access: { has: obj => "setAutomationPolicy" in obj, get: obj => obj.setAutomationPolicy }, metadata: _metadata }, null, _instanceExtraInitializers);
            ...
            __esDecorate(this, null, _latestExtraction_decorators, { kind: "method", name: "latestExtraction", static: false, private: false, access: { has: obj => "latestExtraction" in obj, get: obj => obj.latestExtraction }, metadata: _metadata }, null, _instanceExtraInitializers);
            __esDecorate(this, null, _delete_decorators, { kind: "method", name: "delete", static: false, private: false, access: { has: obj => "delete" in obj, get: obj => obj.delete }, metadata: _metadata }, null, _instanceExtraInitializers);
            __esDecorate(this, null, _latestAudit_decorators, { kind: "method", name: "latestAudit", static: false, private: false, access: { has: obj => "latestAudit" in obj, get: obj => obj.latestAudit }, metadata: _metadata }, null, _instanceExtraInitializers);
        ...
        admissionOpen = true;
        extractionOperations = new Map();
        extractionControllers = new Set();
        automationOperations = new Map();
        pendingRecallAudits = new Map();
        /**
        ...
        constructor(ctx, config) {
            ...
            ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
                ...
                if (state?.privacy === 'durable' && state.modelDisclosureAccepted)
                    return complete;
                return {
                    ...complete,
            ...
            ctx.on('llm/stream', (options, next) => {
                ...
                const pending = options.messages.flatMap((message) => {
                    ...
                    if (audit === undefined || audit.sessionId !== options.sessionId)
                        return [];
            ...
            ctx.on('agent/status', ({ agent, status }) => {
                ...
                for (const [messageId, audit] of this.pendingRecallAudits) {
                    if (audit.sessionId === agent.session.id)
                        this.pendingRecallAudits.delete(messageId);
                }
                this.scheduleAutomaticExtraction(agent);
            });
            ctx.effect(() => async () => {
                this.admissionOpen = false;
                for (const controller of this.extractionControllers)
                    controller.abort();
        ...
        correctionExecution(exec) {
            ...
            if (agent === undefined)
                this.rejectCorrection('A calling Agent is required.', 'CORRECTION_AGENT_REQUIRED');
        ...
        currentEvidence(messages, quote) {
            ...
            if (matched === undefined)
                this.rejectCorrection('A current direct human message is required.', 'CORRECTION_EVIDENCE_INVALID');
        ...
        proposeCorrection(agent, source, args) {
            return this.enqueue(async () => {
                ...
                if (access !== null)
                    this.rejectCorrection(access.code, 'CORRECTION_ACCESS_DENIED');
        ...
        assertCorrectionProposalPresented(agent, candidate, confirmationMessageId) {
            ...
            const proposalMessageIds = new Set(candidate.sources.flatMap(source => source.sessionId === agent.session.id && source.messageId !== undefined ? [source.messageId] : []));
            ...
            const proposalResultIndex = events.findIndex((event, index) => {
                if (index <= proposalIndex || index >= confirmationIndex || event.type !== 'tool/result')
                    return false;
                const call = events.slice(proposalIndex + 1, index).findLast(candidateEvent => candidateEvent.type === 'tool/call'
                    && candidateEvent.data.callId === event.data.message.source.callId);
                if (call?.type !== 'tool/call' || call.data.name !== CORRECTION_TOOL_NAME)
                    return false;
            ...
            return this.enqueue(async () => {
                ...
                try {
                    ...
                    if (request.temporaryDays === undefined)
                        ...
                        candidate.expiresAt = now + request.temporaryDays * DAY_MS;
            ...
            return this.enqueue(async () => {
                ...
                try {
                    ...
                    const alreadyApplied = request.resolution === 'replace-existing'
                        && (target.revisions?.some(revision => revision.action === 'replaced' && revision.relatedMemoryId === candidate.id) ?? false);
                    if (!alreadyApplied) {
        ...
        startExtraction(agent, request, trigger, agentSignal, automaticSourceMessageIds) {
            if (!this.admissionOpen)
                return Promise.reject(new Error('mind-garden-memory: service is disposing'));
            const access = this.modelAccessFailure(agent);
            if (access !== null)
        ...
        scheduleAutomaticExtraction(agent) {
            if (!this.admissionOpen || this.automationOperations.has(agent.session.id))
                return;
            const operation = this.runAutomaticExtraction(agent).catch((error) => {
                if (this.admissionOpen) {
                    this.ctx.logger.warn(`mind-garden-memory: automatic extraction unavailable: ${this.safeDiagnostic(error)}`);
                }
            }).finally(() => {
        ...
        async runAutomaticExtraction(agent) {
            ...
            try {
                maintenance = agent.runMaintenance(async (signal) => {
                    ...
                    let outcome = 'failed';
                    try {
                        const result = await this.startExtraction(agent, {}, 'automatic', signal, new Set(attempt.sourceMessageIds));
                        if (result.ok)
                            outcome = 'completed';
                    }
                    finally {
        ...
        async prepareAutomaticAttempt(agent) {
            ...
            if (available.length < policy.minimumCompletedTurns)
                return null;
        ...
        eligibleCompletedTurns(agent) {
            ...
            for (const event of agent.session.events) {
                ...
                if (candidate.type !== 'mind-garden/safety-assessment')
                    continue;
                ...
                if (candidate.data?.response === 'local'
                    ...
                    excluded.add(candidate.data.turn);
                }
            }
            return [...new Set(agent.session.events.flatMap(event => event.type === 'turn/end'
        ...
        userMessageIdsForTurns(agent, turns) {
            ...
            for (const event of agent.session.events) {
                if (event.type === 'turn/start')
                    ...
                    turn = null;
                else if (event.type === 'user/message' && turn !== null && turns.has(turn))
        ...
        async readRecords() {
            ...
            try {
                return entries.map(([id, value]) => {
                    ...
                    if (record.id !== id)
                        throw new TypeError('vault id differs from authenticated record id');
                    return record;
                });
            }
            catch (error) {
        ...
        requireMemory(records, id) {
            ...
            if (memory === undefined)
                throw new MemoryBusinessError({ code: 'memory-not-found', id });
            return memory;
        }
        ...
        acceptCandidate(candidate, request, now, resolution) {
            ...
            if (request.temporaryDays === undefined)
                delete accepted.expiresAt;
            else
                accepted.expiresAt = now + request.temporaryDays * DAY_MS;
        ...
        async replaceRelatedCandidate(candidate, target, now, options) {
            ...
            if (options.alreadyApplied !== true) {
                ...
                if (options.scope === undefined)
                    ...
                    replacement.scope = options.scope;
                if (candidate.proposalOrigin === undefined)
                    delete replacement.proposalOrigin;
                else
                    replacement.proposalOrigin = candidate.proposalOrigin;
                if (candidate.confidence === undefined)
                    delete replacement.confidence;
                else
                    replacement.confidence = candidate.confidence;
                if (candidate.importance === undefined)
                    delete replacement.importance;
                else
                    replacement.importance = candidate.importance;
                if (candidate.extractionRunId === undefined)
                    delete replacement.extractionRunId;
                else
                    replacement.extractionRunId = candidate.extractionRunId;
                delete replacement.relationship;
                delete replacement.supersededBy;
                if (!temporary)
                    delete replacement.expiresAt;
                else
                    replacement.expiresAt = options.expiresAt ?? now + options.temporaryDays * DAY_MS;
        ...
        requiredText(value, field, maxBytes) {
            if (value.trim().length === 0)
                throw new MemoryBusinessError({ code: 'field-blank', field });
        ...
        async recoverExtraction(agent) {
            const records = await this.readRecords();
            const runs = records.flatMap((record) => record.recordType === 'extraction-run' && record.sessionId === agent.session.id ? [record] : []).sort((left, right) => left.createdAt - right.createdAt || left.id.localeCompare(right.id));
        ...
        async prepareExtraction(agent, request, trigger, automaticSourceMessageIds) {
            ...
            const envelope = buildExtractionEnvelope(messages, memories, this.options.maxExtractionInputBytes, this.options.maxExtractionMemoryBytes, this.options.maxExtractionCandidates);
            if (!envelope.hadHumanText)
                throw new MemoryBusinessError({ code: 'extraction-no-source' });
        ...
        async callExtractionModel(agent, envelope, run, signal) {
            ...
            if (this.extractionFinishFailed(assembler.finish))
                throw new Error('extraction model did not finish completely');
        ...
        extractionCandidates(agent, envelope, proposals, records, runId) {
            ...
            for (const { proposal } of ranked) {
                if (proposal.confidence < this.options.minExtractionConfidence)
                    continue;
                const source = sources.get(proposal.sourceMessageId);
                if (source === undefined || !source.text.includes(proposal.evidenceQuote))
                    continue;
                ...
                if (normalized.length === 0
                    ...
                    || this.forbiddenInference(proposal.content))
                    continue;
                try {
                    ...
                    const duplicate = existing.some(memory => this.normalizedCandidateContent(memory.content) === normalized
                        && memory.sources.some(candidateSource => candidateSource.sessionId === agent.session.id
                ...
                if (accepted.length >= this.options.maxExtractionCandidates)
                    break;
            }
            return accepted;
        }
        ...
        async failExtractionRun(id, failure, rawOutput) {
            const records = await this.readRecords();
            const run = records.find((record) => record.recordType === 'extraction-run' && record.id === id);
            if (run === undefined)
                throw new CorruptMemoryStoreError('Mind Garden extraction audit is missing');
            await this.writeRecord(storedExtractionRunSchema.parse({
                ...run,
                status: 'failed',
                failure,
            ...
            return (async function* (service) {
                for (const [messageId, audit] of pending) {
                    await service.enqueue(async () => {
                        if (service.pendingRecallAudits.get(messageId) !== audit)
                            return;
...
export { MindGardenMemoryService };
