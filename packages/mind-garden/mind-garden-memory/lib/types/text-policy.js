...
const DECLINE_STANDALONE = ['不对', '错了', '否', 'no', 'nope'];
const DECLINE_PHRASES = ['不是这样', '不是这个意思', '并不正确', 'that is wrong', "that's wrong"];
 ...
 */
export function interpretCorrectionDecision(value) {
    ...
    const intent = explicitCancellation
        ? 'cancel'
 ...
 */
export function forbiddenInferenceKind(value) {
    const normalized = normalizedText(value);
    if (containsAnyPhrase(normalized, CLINICAL_DIAGNOSIS_PHRASES))
        return 'clinical-diagnosis';
    if (containsAnyPhrase(normalized, PERSONALITY_LABEL_PHRASES))
        return 'personality-label';
    if (containsAnyPhrase(normalized, HIDDEN_CAUSE_PHRASES))
        return 'hidden-cause';
    return null;
}
//# sourceMappingURL=text-policy.js.map