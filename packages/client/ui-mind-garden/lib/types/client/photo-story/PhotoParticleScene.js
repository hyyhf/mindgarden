...
function photoParticleAttributes(image, config) {
    ...
    for (let index = 0; index < width * height; index++) {
        if (pixelView.getUint8(index * 4 + 3) > 20 && seeded(index) <= config.rendering.density)
            selected.push(index);
    }
    if (selected.length === 0)
        throw new Error('photo-has-no-visible-pixels');
    ...
    const pixelLuma = (index) => {
        const offset = Math.min(index, width * height - 1) * 4;
        return (pixelView.getUint8(offset) * 0.2126
            + pixelView.getUint8(offset + 1) * 0.7152
            + pixelView.getUint8(offset + 2) * 0.0722) / 255;
    };
...
export function mountPhotoParticleScene(host, image, initialConfig, reducedMotion) {
    ...
    const movePointer = (event) => {
        ...
        uniforms.pointer.value.set(((event.clientX - rect.left) / Math.max(1, rect.width) - 0.5) * (attributes.aspect >= 1 ? 12 : 12 * attributes.aspect), (0.5 - (event.clientY - rect.top) / Math.max(1, rect.height)) * (attributes.aspect >= 1 ? 12 / attributes.aspect : 12));
    ...
    const pointerDown = (event) => {
        ...
        if (config.interaction.clickBurst)
            burst = 1;
    };
